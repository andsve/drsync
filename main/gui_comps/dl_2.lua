...
function dirtylarry.radio(self, node, action_id, action, id, value, cleanup_id)

	local node_bg = gui.get_node(node .. "/larryradio")
	local node_label = gui.get_node(node .. "/larrylabel")

	if cleanup_id and dirtylarry._cleanup[cleanup_id] then
		table.insert(dirtylarry._cleanup[cleanup_id], node_bg)
	elseif cleanup_id then
		dirtylarry._cleanup[cleanup_id] = {node_bg}
	end

	local hit = hit_test(self, node_bg, action_id, action) or hit_test(self, node_label, action_id, action)
	if hit and action.released then
		if cleanup_id and value ~= id then
			for i,v in ipairs(dirtylarry._cleanup[cleanup_id]) do
				gui.play_flipbook(v, "radio_normal")
			end
			dirtylarry._cleanup[cleanup_id] = nil
		end
		value = id
	end
...
function dirtylarry.input(self, node, action_id, action, type, empty_text)
	...
	s.x = s.x - 4-dirtylarry._input_pad
	s.y = s.y - 4
	...
	if hit_test(self, node_bg, action_id, action) then
		...
		gui.animate(node_cursor, "size", vmath.vector3(4, dirtylarry._input_pad, 0), gui.EASING_OUTCUBIC, 0.2)
